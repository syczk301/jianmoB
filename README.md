...
本仓库按用户选择发布轻量版本：包含四个代码文件、依赖清单、原始题目数据、参数、中文图表及计算汇总。全精度状态轨迹 `work/runs`、虚拟环境、逐候选缓存与旧压缩备份由 `.gitignore` 排除。文中“全部历史记录”和 `archive` 的说明对应整理时的本地目录布局，不代表这些文件已上传，也不保证它们在后续本地清理后仍存在。

发布时可恢复的109个主要工况已另存为本地 `archive/主要轨迹本地恢复备份_00d2197.zip`，压缩包校验通过。该备份未上传，不包含全部历史搜索工况；轻量仓库中的汇总记录不等同于完整状态轨迹。
